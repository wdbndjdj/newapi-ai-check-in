...
工作流每天北京时间 **00:00** 自动执行，也支持在 Actions 页面手动运行。

首次配置步骤（20 个账号）：

1. 本地分别运行 `uv run python nofx_checkin.py --capture-state nofx-state-1.json` 至
   `nofx-state-20.json`（第一个文件也可命名为 `nofx-state.json`）。
2. 在打开的浏览器中输入邮箱并完成 Magic Link 登录，回到终端按回车保存状态。
3. 将 20 个文件分别编码为 base64，写入 `production` Environment Secrets
   `NOFX_STORAGE_STATE_B64_1` 至 `NOFX_STORAGE_STATE_B64_20`：
